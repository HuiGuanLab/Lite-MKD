...
class strmclassifiers_resnet18_sup(nn.Module):
    '''
    TRX分类器
    ...
    '''
    def __init__(self, args, ):
        super(strmclassifiers_resnet18_sup, self).__init__()
        self.train()
        self.args = args       
        self.transformers = TemporalCrossTransformer(args, 2)
        self.DistanceLoss = DistanceLoss(args, 2)
        self.SupportDK=SupportDK(args)
                           
    def forward(self,context_feature, context_labels, target_feature):
        
        context_feature_pat = context_feature['distance']
        context_feature_fr1 = context_feature['trx1']
        context_feature_fr2 = context_feature['trx2']
        
        target_feature_pat = target_feature['distance']
        target_feature_fr1 = target_feature['trx1']
        target_feature_fr2 = target_feature['trx2']
        

        all_logits_pat=self.DistanceLoss(context_feature_pat, context_labels, target_feature_pat,self.args.device)['logits']
        all_logits_fr1=self.transformers(context_feature_fr1, context_labels, target_feature_fr1)['logits']
        all_logits_fr2=self.transformers(context_feature_fr2, context_labels, target_feature_fr2)['logits']

        all_logits_sup=self.SupportDK(context_feature_fr2, context_labels, target_feature_fr2)['logits']
        
        all_logit={'pat': all_logits_pat, 'fr1': all_logits_fr1,'sup':all_logits_sup,'fr2':all_logits_fr2}
