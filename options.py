...
def parse_common_args(parser):  #公共参数，一般模型都会使用
        gpu_device = 'cuda'
        ...
        parser.add_argument("--cfg",  default={
            ...
            'fcwsl_aerfa':0.5,
            'fcwsl_beta':1
        }, help="experiment Hyperparameter")  #本次实验的简单描述，搞清楚这个实验干了什么

        return parser

def parse_train_args(parser):   #训练使用的参数
    parser = parse_common_args(parser)
    
    #训练次数、重连参数、什么时候调用测试、学习率、优化器
    ...
    parser.add_argument('--test_iters', nargs='+', type=int, help='iterations to test at. Default is for ssv2 otam split.', default=[10000,15000,20000,30000,35000,40000,50000,60000,70000,80000,90000,100000])
...
def parse_test_args(parser):   #测试使用的参数
    parser = parse_common_args(parser)
    #测试样本、测试模型地址
...
def args_cheak(args): 
    '''参数检查、对参数做一些后续的添加和改动也在这里'''
    
    if args.checkpoint_dir == None:   #选择权重文件夹
    ...
    verify_checkpoint_dir(args.checkpoint_dir,args.resume_from_checkpoint, False)  #检查权重文件夹
            
    return args
        
def verify_checkpoint_dir(checkpoint_dir, resume, test_mode):  #验证权重文件夹
        if resume:  # verify that the checkpoint directory and file exists   验证权重文件是否存在
...
def get_data_path(args):    #数据集地址,到数据文件夹为止，数据文件夹，比如“kinect_l8,或者kinect_feature”，里面存放每个模态或者原始视频的数据，比如rgb，flow。
                        #从这层开始所有文件夹的目录是一样的   
    if args.dataset == "ssv2":
            ...
            args.teacher_RGB = os.path.join("/home/zty/204/data","kinetics/feature/rgb")
            # args.depth_path = os.path.join("data", "kinetics/feature/depth")
            # args.flow_path = os.path.join("data", "kinetics/feature/flow")
            # args.rgb_path = os.path.join("data", "kinetics/feature/rgb")
            ...
            args.teacher_RGB = os.path.join("/home/zty/204/data","ucf101/feature/RGB/trm/rgb")
            ...
            args.teacher_RGB = os.path.join("/home/zty/204/data","hmdb/feature/RGB/meta/rgb")

            
            
    return args

def get_train_args():  #获得训练参数
...
def prepare_train_args_wandb():  #最终训练参数
    #对基本的参数进行检验和增加数据地址
    args = get_train_args()
    args = get_data_path(args)
    args.debug=False
