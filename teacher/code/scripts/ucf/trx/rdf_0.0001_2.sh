...
export lr=0.0001
export checkpoint_dir=work/${dataset}/fusion/trx/final/lr${lr}_tempset2 # save dir
...
CUDA_VISIBLE_DEVICES=${DEVICES} python3 multi_fusion.py -c ${checkpoint_dir} --query_per_class 4 --shot 5 --way 5 \
    ...
    -lr ${lr} --model ${model} --m2 ${m2} --m3 ${m3} --m4 ${m4} --shirt_num ${shirt_num} --trans_num 2 --temp_set 2
