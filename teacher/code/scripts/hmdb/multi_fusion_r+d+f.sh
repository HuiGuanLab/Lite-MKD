...
export lr=0.00005
export checkpoint_dir="work/hmdb/fusion/trx/multi_fusion_r+d+f_lr${lr}" # save dir
...
CUDA_VISIBLE_DEVICES=${DEVICES} python3 multi_fusion.py -c ${checkpoint_dir} --query_per_class 4 --shot 5 --way 5 \
    ...
    --temp_set 2 -lr ${lr} --model ${model} --m2 ${m2} --m3 ${m3} --m4 ${m4} --shirt_num ${shirt_num} --trans_num 2
